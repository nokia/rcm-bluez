...
//static disconnect_cb_t req_disconnect_cb = NULL;
...
/*
void proxy_disconnect_cb_register( disconnect_cb_t cb)
{
	TRACE_FUNCTION;

	if(req_disconnect_cb == NULL)
		req_disconnect_cb = cb;
	else
		DBG("RCM DBG: disconnect init callback is already set!");
}

static void proxy_disconnect_cb_call( disconnect_cb_t	cb,
								void	   *userdata)
{
	TRACE_FUNCTION;
	...
	if(cb != NULL)
		...
		DBG("RCM DBG: disconnect init callback is not set!");
}
*/
void rcm_gdbus_send_signal( char		*signal_name,
...
static int rcm_gdbus_find_filter_element( gconstpointer a,
								gconstpointer b)
{
	TRACE_FUNCTION;
                    ...
                    gpointer               user_data)
{
	...
	if (g_strcmp0 (method_name, "DisconnectAll") == 0)
	{
		proxy_disconnect_cb_call(req_disconnect_cb, NULL);
		return;
	}*/
...
//			token = strtok(NULL, delimiters);
			...
			GSList * found_el = g_slist_find_custom(init_filter, token, rcm_gdbus_find_filter_element);
...
// cf. GetInitFilter to fill a corresponding GSList in ActiveConnection structure
		...
		rcm_get_client_filter(mac, &g_var_builder);
		g_variant_builder_close(&g_var_builder);
		GVariant *string_array = g_variant_builder_end(&g_var_builder);
...
//		DBG("mac = %s\n", mac);
		if(!rcm_connection_exists(mac)) return;
...
//			DBG("info = %s\n", str);
			...
			GSList * found_el = rcm_check_autorized(mac, token);
/*
			token = strsep(&str_copy, delimiters);
...
*/
			...
			{
//				DBG("element is not found, add it\n");
				rcm_authorize_device(mac, filter_entry);
			}
			else
			{
//				DBG("element is found, don't add it again\n");
