...
// RCM
static start_write_cb_t start_write_cb = NULL; // To distinguish passive and active discovery, better is to have a list of callback functions
//start_write_request
void start_write_cb_register(start_write_cb_t cb)
{
	DBG("RCM DBG: setting start_write_cb_t callback");
	if(start_write_cb == NULL)
		start_write_cb = cb;
	else
		DBG("RCM DBG: start_write_cb_t callback is already set!");
}

static void start_write_cb_call(start_write_cb_t cb, unsigned int id, uint16_t value_handle)
{
	DBG("RCM DBG: call start_write_cb_t callback");
	if(cb != NULL)
		(*cb)(id, value_handle);
	else
		DBG("RCM DBG: start_write_cb_t callback is not set!");
}

static write_result_cb_t rcm_write_result_cb = NULL; // To distinguish passive and active discovery, better is to have a list of callback functions

void write_result_cb_register(write_result_cb_t cb)
{
	DBG("RCM DBG: setting write_result_cb_t callback");
	if(rcm_write_result_cb == NULL)
		rcm_write_result_cb = cb;
	else
		DBG("RCM DBG: write_result_cb_t callback is already set!");
}

static void write_result_cb_call(write_result_cb_t cb, unsigned int id)
{
	DBG("RCM DBG: call write_result_cb_t callback");
	if(cb != NULL)
		(*cb)(id);
	else
		DBG("RCM DBG: write_result_cb_t callback is not set!");
}

static notify_cb_t rcm_notify_cb = NULL; // To distinguish passive and active discovery, better is to have a list of callback functions
//start_write_request
void notify_cb_register(notify_cb_t cb)
{
	DBG("RCM DBG: setting notify callback");
	if(rcm_notify_cb == NULL)
		rcm_notify_cb = cb;
	else
		DBG("RCM DBG: notify callback is already set!");
}

static void notify_cb_call(notify_cb_t cb, uint16_t value_handle, uint16_t value_len, const uint8_t *value)
{
	DBG("RCM DBG: call notify callback");
	if(cb != NULL)
		(*cb)(value_handle, value_len, value);
	else
		DBG("RCM DBG: notify callback is not set!");
}
					...
					uint8_t att_ecode, void *user_data)
{
	...
	write_result_cb_call(rcm_write_result_cb, op->id);
					...
					async_dbus_op_complete_t complete)
{
	...
	DBG("start_write_request handle = %02x\n", handle);
	start_write_cb_call(start_write_cb, op->id, handle);
...
static DBusMessage *characteristic_write_value(DBusConnection *conn,
					DBusMessage *msg, void *user_data)
{
	DBG("[characteristic_write_value] Entered");
	 ...
	 */
	DBG("[characteristic_write_value] (chrc->ext_props = %d BT_GATT_CHRC_EXT_PROP_RELIABLE_WRITE) = %d", chrc->ext_props, BT_GATT_CHRC_EXT_PROP_RELIABLE_WRITE);
	...
	DBG("[characteristic_write_value] (chrc->props = %d BT_GATT_CHRC_PROP_WRITE) = %d", chrc->props, BT_GATT_CHRC_PROP_WRITE);
	if (chrc->props & BT_GATT_CHRC_PROP_WRITE) {
		uint16_t mtu;

		DBG("[characteristic_write_value] inside here");
		...
		if (chrc->write_op){
			DBG("[characteristic_write_value] (chrc->write_op = %d", chrc->write_op);
			return NULL;
		}
	}
	...
	DBG("[characteristic_write_value] bt_gatt_client_write_without_response");
					...
					uint16_t length, void *user_data)
{
	...
	printf("notify_cb value_handle = %02x, value length = %d value: \n", value_handle, length);
	for(int i=0; i<length; i++)
		printf("%02x ", value[i]);
	printf("\n");

	notify_cb_call(rcm_notify_cb, value_handle, length, value);
	/*
...
static void register_notify_cb(uint16_t att_ecode, void *user_data)
{
	...
	write_result_cb_call(rcm_write_result_cb, client->notify_id);
...
static DBusMessage *characteristic_start_notify(DBusConnection *conn,
					DBusMessage *msg, void *user_data)
{
	DBG("characteristic_start_notify\n");
	...
	if (!(chrc->props & BT_GATT_CHRC_PROP_NOTIFY ||
				chrc->props & BT_GATT_CHRC_PROP_INDICATE)){
		DBG("characteristic_start_notify : property not supported\n");
		return btd_error_not_supported(msg);
	}
	/* Each client can only have one active notify session. */
	client = queue_find(chrc->notify_clients, match_notify_sender, sender);
	if (client){
		start_write_cb_call(start_write_cb, client->notify_id, chrc->value_handle + 1 );
		write_result_cb_call(rcm_write_result_cb, client->notify_id);
		return client->notify_id ?
	...
	DBG("characteristic_start_notify : calling bt_gatt_client_register_notify\n");
