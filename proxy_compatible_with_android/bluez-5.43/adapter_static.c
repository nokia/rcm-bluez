...
{
	DBG("NATALYA DBG: DISCOVERY STARTED !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
	struct btd_adapter *adapter = user_data;
	...
	for (l = discovery_filter; l != NULL && got_match != true;
		...
		if (!item->uuids)
			...
			for (m = item->uuids; m != NULL && got_match != true;
				 ...
				 * uuid.
				 */
				DBG("NATALYA DBG: START COMPARISON. Name %s SERVICE UUID %s, eir_services %s, rssi = %d",
						eir_data->name, (char *) m->data, (char *) eir_data->services->data, item->rssi);
				if (g_slist_find_custom(eir_data->services,
							m->data,
							g_strcmp) != NULL)
				{
					DBG("NATALYA DBG: NORMALLY SHOULD MATCH. Name %s SERVICE UUID %s, eir_services %s, rssi = %d",
							eir_data->name, (char *) m->data, (char *) eir_data->services->data, item->rssi);
					got_match = true;
				}
			}
