...
void BTA_GATTC_ServiceSearchRequest(uint16_t conn_id, tBT_UUID* p_srvc_uuid) {
	  APPL_TRACE_DEBUG("%s", __func__);
	  // RCM
//	  APPL_TRACE_DEBUG("uuid len = %d", p_srvc_uuid->len);
//	  for(int i=0; i<p_srvc_uuid->len; i++)
//		  APPL_TRACE_DEBUG("%02x ", p_srvc_uuid->uu);
...
tBTA_GATT_STATUS BTA_GATTC_RegisterForNotifications(tBTA_GATTC_IF client_if,
  ...
  if(rcm_device_exists(bda)){
	  rcm_register_for_notif(true);
  }
