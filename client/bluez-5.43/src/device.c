...
static int device_remote_browse_gatt(struct btd_device *device, DBusMessage *msg);
...
{
	struct btd_device *device = user_data;
	printf("[disconnect_all] device->connect=%s, device->disconnect=%s\n", device->connect, device->disconnect);
...
{
	GSList *l;
	DBG("NATALYA DBG: btd_device_connect_services");
					...
					DBusMessage *msg, const char *uuid)
{
	...
	DBG("NATALYA DBG: CONNECT ProfileS");
...
// BEGIN +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
static void connect_cb_register(struct btd_device *device, connect_cb_t cb)
{
	DBG("NATALYA DBG: setting connect callback");
	device->connect_callback = cb;
}

static void connect_cb_call(connect_cb_t cb, void *userdata)
{
	DBG("NATALYA DBG: register callback");
	if(cb != NULL)
		(*cb)(userdata);
	else
		DBG("NATALYA DBG: connect callback is not set!");
}

void send_dbus_message(struct btd_device * device)
{
	DBG("NATALYA DBG: SEND DBUS MESSAGE ON CONNECT");
	DBusMessage *reply;
	if (device->connect)
	{
		char addr[18];
		ba2str(&device->bdaddr, addr);
		DBG("NATALYA DBG: SEND DBUS MESSAGE ON CONNECT: sending connect message! msg %p addr = %s", device->connect, addr);
	...
	else
	{
		char addr[18];
		ba2str(&device->bdaddr, addr);
		DBG("NATALYA DBG: SEND DBUS MESSAGE ON CONNECT: no connect message! msg %p addr = %s", device->connect, addr);
	}
}
// END +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//HUI
void device_set_owner_flag(struct btd_device *device, const char *sender)
{
	if (!device)
		return;
	device->owner = g_strdup(sender);
}
							...
							void *user_data)
{
	...
	DBG("NATALYA DBG: dev_connect, got DBus \"Connect\" message.");
	DBG("HUI DBG: device->from_bluetooth = %d, device->from_rcm = %d", dev->from_bluetooth, dev->from_rcm);

	const char *sender = dbus_message_get_sender(msg);
	DBG("sender %s", sender);

	//HUI
	device_set_owner_flag(dev, sender);

	if(dev->from_bluetooth)
	{
		uint8_t bdaddr_type;

		DBG("NATALYA DBG: dev_connect, device is not remote!");
		...
		if (dev->bredr_state.connected) {
			...
			DBG("NATALYA DBG: dev_connect in else");
			bdaddr_type = select_conn_bearer(dev);
		}
		if (bdaddr_type != BDADDR_BREDR) {
			int err;

			DBG("NATALYA DBG: dev_connect: bdaddr_type != BDADDR_BREDR");
			...
			DBG("NATALYA DBG: dev_connect: call device_connect_le, err = %d", err);
			if (err < 0)
				return btd_error_failed(msg, strerror(-err));
	...
	else if(dev->from_rcm && !dev->from_bluetooth) // Device is a remote one, a connection should be managed differently
	{
		int err;

		DBG("NATALYA DBG: remote dev_connect: bdaddr_type != BDADDR_BREDR");
		if (dev->le_state.connected)
			return dbus_message_new_method_return(msg);
		...
		err = remote_device_connect_le(dev); // TODO: adjust this function or create a new one doing similar things but that doesn't pass by io loop
		DBG("NATALYA DBG: remote dev_connect: call remote_device_connect_le, err = %d", err);
		if (err < 0)
			return btd_error_failed(msg, strerror(-err));
		...
		DBG("NATALYA DBG: dev_connect, device is remote, we shouldn't follow the standard connect process, "
																				   "dev->connect = %p msg %p, addr= %s",
																					dev->connect, msg, addr);

		connect_cb_call(dev->connect_callback, dev);

		return NULL;//dbus_message_new_method_return(msg);
	}
	return NULL;
}
							...
							void *user_data)
{
	...
	DBG("NATALYA DBG: CONNECT Profile, uuid = %s", uuid);
...
static void browse_request_complete(struct browse_req *req, uint8_t bdaddr_type,
									int err)
{
	DBG("NATALYA : [browse_request_complete]");
							...
							const char *peer)
{
	...
	DBG("NATALYA DBG: cache filename =  STORAGEDIR = %s \"/%s/cache/%s\"", STORAGEDIR, local, peer);
	key_file = g_key_file_new();
	g_key_file_load_from_file(key_file, filename, 0, NULL);
				...
				const char *address)
{
	...
	DBG("NATALYA DBG: [device_new] address %s, db = %p", address, device->db);
...
struct btd_device *device_create_remote(struct btd_adapter *adapter,
					const bdaddr_t *bdaddr, uint8_t bdaddr_type, bool remote, connect_cb_t cb)
{
	DBG("NATALYA : [device_create_remote]");
	struct btd_device *device;
	const bdaddr_t *sba;
	char src[18], dst[18];
	char *str;
...
//	DBG("NATALYA DBG: device_create: device remote? (from structure) %d", device->remote);
	connect_cb_register(device, cb);
				...
				const bdaddr_t *bdaddr, uint8_t bdaddr_type)
{
	...
	// Main difference
	// Use this function to create the nearby devices only
	// This is also the one used by default
//	device->remote = false;
...
//	if(device->remote)
	if(device->from_rcm && !device->from_bluetooth)
		btd_gatt_remote_client_ready(device->client_dbus);
	else if(device->from_bluetooth)
...
//	if(device->remote)
	if(device->from_rcm && !device->from_bluetooth)
		device->client = bt_gatt_remote_client_new(device->db, device->att,
				device->att_mtu); // XXX look inside gatt_client_init inside the bt_gatt_remote_client_new
	else if(device->from_bluetooth)
...
bool device_attach_remote_att(struct btd_device *dev)
{
	DBG("NATALYA : [device_attach_remote_att]");
	GAttrib *attrib;
	uint16_t mtu;
	...
	dev->att_mtu = BT_ATT_MAX_LE_MTU;
	attrib = g_remote_attrib_new(BT_ATT_DEFAULT_LE_MTU, false);
	if (!attrib) {
...
// Do we really need this for the remote device?
	if (dev->local_csrk)
...
// till here
	...
	DBG("NATALYA DBG: device_attach_remote_att dev->db = %p", dev->db);
	if (gatt_db_isempty(dev->db))
		load_gatt_db(dev, srcaddr, dstaddr); // This should be called when the cache is retrieved from the server

	gatt_client_init(dev);	// XXX In progress
//	gatt_server_init(dev, btd_gatt_database_get_db(database)); // XXX TODO

	// Natalya
//		notify_client_ready(dev->client, true, 0);
//	if(dev->remote)
	if(dev->from_rcm && !dev->from_bluetooth)
	{
		remote_notify_client_ready(dev->client, true, 0);
	}
	// till here
...
// BEGIN +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
void att_remote_connect_cb(GIOChannel *io, GError *gerr, gpointer user_data)
{
	DBG("call att_remote_connect_cb!!!!");
	struct btd_device *device = user_data;
	DBusMessage *reply;
	device->att_io = NULL;
	uint8_t io_cap;
	...
	/*
	 * This part of the code should also be reimplemented with callbacks
	 * The idea is that instead of io loop we will use our connection with the server to get the necessary information
	 */

 	if (!device_attach_remote_att(device))
 	{
 		DBG("[att_remote_connect_cb] device_attach_remote_att == true");
		goto done;
 	}
	...
	DBG("HUI: device->bonding->agent->owner = %s ", device->bonding->agent->owner);
	err = adapter_remote_create_bonding(device->adapter, &device->bdaddr,
...
*/
	DBG("[att_remote_connect_cb] check for errors, should be 0, really = %d", err);
	if (!err)
	{
		DBG("[att_remote_connect_cb] no errors, device_remote_browse_gatt should be called");
		device_remote_browse_gatt(device, NULL);
	}

	if (device->connect) {
		if (err < 0)
		{
			DBG("DBus reply is not ok, the error = %d", err);
			reply = btd_error_failed(device->connect,
					strerror(-err));
		}
		else
		{
			DBG("Send Dbus OK message");
			reply = dbus_message_new_method_return(device->connect);
		}
...
//	DBG("NATALYA DBG: %s", gerr->message);
	DBG("NATALYA DBG: att_connect_cb");
	...
	if (gerr) {
		...
		if(device->connect)
		{
			DBG("HUI: Connection error, try to connect with RCM!!!");
			device_set_bluetooth_flag(device, false);
			dev_connect(dbus_conn, device->connect, device);
			return;
		}
...
// BEGIN +++++++++++++++++++++++++++++++++++++++++++++++++++++++
connect_cb_t waiting_for_cache_cb = NULL;

void cache_update_cb_register(connect_cb_t cb)
{
	DBG("NATALYA DBG: setting connect callback");
	if(waiting_for_cache_cb == NULL)
		waiting_for_cache_cb = cb;
	else
		DBG("NATALYA DBG: cache update cb is already set!");
}

static void cache_update_cb_call(connect_cb_t cb, void *userdata)
{
	DBG("NATALYA DBG: register callback");
	if(cb != NULL)
		(*cb)(userdata);
...
int remote_device_connect_le(struct btd_device *dev)
{

	DBG("NATALYA : [remote_device_connect_le]");
...
// Make a callback here instead... The att_remote_connect_cb will be called when the cache is successfully updated
// (that's because the gatt db should be loaded from the cache only and nothing else will be done, no real service discovery!)
//	bt_io_remote_connect(att_remote_connect_cb, dev, NULL, &gerr);

	cache_update_cb_call(waiting_for_cache_cb, dev);
	return 0;
}

bool device_is_remote(struct btd_device *dev)
{
	if(dev->from_bluetooth)
		return false;
...
int device_connect_le(struct btd_device *dev)
{
	...
	if (io == NULL) {
		...
		g_error_free(gerr);
		DBG("NATALYA DBG: after bt_io_connect, if we are here, it's NOT ok");
		return -EIO;
	}

	DBG("NATALYA DBG: after bt_io_connect, if we are here, everything is ok");
...
static int device_remote_browse_gatt(struct btd_device *device, DBusMessage *msg)
{
	DBG("NATALYA : [device_remote_browse_gatt]");
	struct btd_adapter *adapter = device->adapter;
	struct browse_req *req;
	...
	if (!req)
	{
		DBG("NATALYA DBG : [device_remote_browse_gatt] Device or ressource busy");
		return -EBUSY;
	}
	...
	if (device->client) {
		 ...
		 * to become ready.
		 */
		// Natalya: I think there is no need to keep it for remote device connection
/*		if (!bt_gatt_client_is_ready(device->client))
			return 0;
*/
		/*
	...
	bt_io_remote_connect(att_remote_connect_cb, device, NULL, NULL);
...
static int device_browse_gatt(struct btd_device *device, DBusMessage *msg)
{
	DBG("NATALYA : [device_browse_gatt]");
	struct btd_adapter *adapter = device->adapter;
	struct browse_req *req;
...
//HUI
void device_set_bluetooth_flag(struct btd_device *device, bool from_bluetooth)
{
	if (!device)
		return;
	device->from_bluetooth = from_bluetooth;
}

void device_set_rcm_flag(struct btd_device *device, bool from_rcm)
{
	if (!device)
		return;
	device->from_rcm = from_rcm;
}

void device_set_callback(struct btd_device *device, connect_cb_t cb)
{
	if (!device)
		return;
	...
	{
		printf("HUI: [device_set_callback]Device connect callback is already set!\n");
		return;
	}
}

bool device_from_bluetooth(struct btd_device *device)
{
...
bool device_from_rcm(struct btd_device *device)
{
	if(device->from_rcm)
		return true;
...
// BEGIN +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
void device_remote_bonding_complete(struct btd_device *device, uint8_t bdaddr_type)
{
	struct bonding_req *bonding = device->bonding;
	...
	if (bonding) {
		...
		if (bdaddr_type != BDADDR_BREDR)
			device_remote_browse_gatt(device, bonding->msg);	// XXX!!!

		bonding_request_free(bonding);
	}
	else
		DBG("NATALYA DBG: [device_remote_bonding_complete] device->bonding == false!");
}
// END +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
struct bt_gatt_client *btd_device_get_gatt_client(struct btd_device *device)
{
	...
	DBG("[btd_device_get_gatt_client] Success");
