...
static void bta_dm_observe_results_cb(tBTM_INQ_RESULTS* p_inq, uint8_t* p_eir,
  ...
  result.inq_res.bd_addr = p_inq->remote_bd_addr;
  result.inq_res.rssi = p_inq->rssi;
  APPL_TRACE_DEBUG("%s %d", __func__, p_inq->rssi);

  result.inq_res.ble_addr_type = p_inq->ble_addr_type;
  APPL_TRACE_DEBUG("%s %d", __func__, p_inq->ble_addr_type);

  result.inq_res.inq_result_type = p_inq->inq_result_type;
  APPL_TRACE_DEBUG("%s %d", __func__, p_inq->inq_result_type);

  result.inq_res.device_type = p_inq->device_type;
  APPL_TRACE_DEBUG("%s %d", __func__, p_inq->device_type);

  result.inq_res.flag = p_inq->flag;
  result.inq_res.ble_evt_type = p_inq->ble_evt_type;
  APPL_TRACE_DEBUG("%s %d", __func__, p_inq->ble_evt_type);

  result.inq_res.ble_primary_phy = p_inq->ble_primary_phy;
  APPL_TRACE_DEBUG("%s %d", __func__, p_inq->ble_primary_phy);

  result.inq_res.ble_secondary_phy = p_inq->ble_secondary_phy;
  APPL_TRACE_DEBUG("%s %d", __func__, p_inq->ble_secondary_phy);

  result.inq_res.ble_advertising_sid = p_inq->ble_advertising_sid;
  APPL_TRACE_DEBUG("%s %d", __func__, p_inq->ble_advertising_sid);

  result.inq_res.ble_tx_power = p_inq->ble_tx_power;
  APPL_TRACE_DEBUG("%s %d", __func__, p_inq->ble_tx_power);

  result.inq_res.ble_periodic_adv_int = p_inq->ble_periodic_adv_int;
  APPL_TRACE_DEBUG("%s %d", __func__, p_inq->ble_periodic_adv_int);
  ...
  if (p_inq_info != NULL) {
    /* initialize remt_name_not_required to false so that we get the name by
     * default */
    APPL_TRACE_DEBUG("%s p_inq_info != NULL result.inq_res.remt_name_not_required = %d", __func__, result.inq_res.remt_name_not_required);
    result.inq_res.remt_name_not_required = false;
  }
  ...
  if (p_inq_info) {
    /* application indicates if it knows the remote name, inside the callback
     copy that to the inquiry data base*/
	APPL_TRACE_DEBUG("%s result.inq_res.remt_name_not_required = %d", __func__, result.inq_res.remt_name_not_required);
    if (result.inq_res.remt_name_not_required)
      p_inq_info->appl_knows_rem_name = true;
  }
}
...
void bta_dm_add_ble_device(tBTA_DM_MSG* p_data) {
  ...
  if (!BTM_SecAddBleDevice(p_data->add_ble_device.bd_addr, NULL,
	  ...
	  APPL_TRACE_DEBUG("%s Error BTM_SecAddBleDevice false", __func__);
