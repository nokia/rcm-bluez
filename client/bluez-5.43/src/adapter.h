...
typedef void (*discovering_cb_t)(void *adapter, const void *sender);
typedef void (*pairing_cb_t)(void *userdata);
//HUI
typedef void (*filter_cb_t)(void *adapter, void *discovery_filter, const void *sender);
typedef void (*disconnect_cb_t)(void *userdata);
...
void pair_remote_device_complete(const void *param, void *user_data);
...
void remotely_connected_device(char *addr,//const bdaddr_t *bdaddr,
							   uint8_t bdaddr_type,
							   const uint8_t *eir,
							   uint16_t eir_len,
							   void *user_data);

int adapter_remote_create_bonding(struct btd_adapter *adapter,
								  const bdaddr_t *bdaddr,
								  uint8_t addr_type,
								  uint8_t io_cap);
