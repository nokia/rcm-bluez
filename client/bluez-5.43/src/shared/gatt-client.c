...
// BEGIN
void remote_notify_client_ready(struct bt_gatt_client *client, bool success,
		uint8_t att_ecode)
{
...
static bool gatt_client_init(struct bt_gatt_client *client, uint16_t mtu)
{
	...
	if (!op)
	{
		printf("[gatt_client_init] Discovery op can't be created\n");
		return false;
	}
	...
	{
		printf("[gatt_client_init] Now we should go to service discovery\n");
		goto discover;
	}

	printf("[gatt_client_init] Shouldn't pass here for remote connection\n"); // because I use the BT_ATT_DEFAULT_LE_MTU
...
// BEGIN +++++++++++++++++++++++++++++++++++++++++++++++++++++++++
static struct bt_gatt_client *gatt_remote_client_new(struct gatt_db *db,
							struct bt_att *att)
{
	struct bt_gatt_client *client;

	client = new0(struct bt_gatt_client, 1);
	client->disc_id = bt_remote_att_register_disconnect(att, att_disconnect_cb,
								client, NULL);
	if (!client->disc_id)
		goto fail;
	...
	client->notify_id = bt_remote_att_register(att, BT_ATT_OP_HANDLE_VAL_NOT,
						notify_cb, client, NULL);
	if (!client->notify_id)
		goto fail;

	client->ind_id = bt_remote_att_register(att, BT_ATT_OP_HANDLE_VAL_IND,
						notify_cb, client, NULL);
	if (!client->ind_id)
		goto fail;
...
// BEGIN +++++++++++++++++++++++++++++++++++++++++
struct bt_gatt_client *bt_gatt_remote_client_new(struct gatt_db *db,
							struct bt_att *att,
							uint16_t mtu)
{
	printf("Enter the [bt_gatt_remote_client_new] \n");

	struct bt_gatt_client *client;

	if (!att || !db)
	{
		printf("NATALYA DBG: [bt_gatt_remote_client_new] bt_gatt_client_new !att || !db\n");
		return NULL;
	}

	client = gatt_remote_client_new(db, att);
	if (!client)
	{
		printf("NATALYA DBG: [bt_gatt_remote_client_new] bt_gatt_client_new !client\n");
		return NULL;
	}
// Normally we recover the services from the retrieved cache

	if (client->in_init || client->ready)
	{
		printf("It seems that the GATT client is already initialized and ready! \n");
	}
	else
	{
		printf("[bt_gatt_remote_client_new] Check mtu = %d\n", mtu);
		mtu = MAX(BT_ATT_DEFAULT_LE_MTU, mtu);
		printf("[bt_gatt_remote_client_new] Check if MTU needs to be sent, MTU_DEFAULT = %d, mtu = %d\n", BT_ATT_DEFAULT_LE_MTU, mtu);
		if (mtu == BT_ATT_DEFAULT_LE_MTU)
		{
			printf("[bt_gatt_remote_client_new] mtu == BT_ATT_DEFAULT_LE_MTU\n");
			client->in_init = true;
		}
	}
/*
	if (!gatt_client_init(client, mtu)) { // XXX
		printf("NATALYA DBG: bt_gatt_client_new !gatt_client_init\n");
		bt_gatt_client_free(client);
		return NULL;
	}
*/
							...
							uint16_t mtu)
{
	...
	if (!att || !db)
	{
		printf("NATALYA DBG: bt_gatt_client_new !att || !db\n");
		return NULL;
	}
	...
	if (!client)
	{
		printf("NATALYA DBG: bt_gatt_client_new !client\n");
		return NULL;
	}
...
//BEGIN
struct bt_gatt_client *bt_gatt_remote_client_clone(struct bt_gatt_client *client)
{
	struct bt_gatt_client *clone;

	if (!client)
	{
		printf("[bt_gatt_remote_client_clone] !client\n");
		return NULL;
	}

	clone = gatt_remote_client_new(client->db, client->att);
	if (!clone)
	{
		printf("[bt_gatt_remote_client_clone] !clone\n");
		return NULL;
	}
...
struct bt_gatt_client *bt_gatt_client_clone(struct bt_gatt_client *client)
{
	...
	if (!client)
	{
		printf("[bt_gatt_client_clone] !client\n");
		return NULL;
	}
	...
	if (!clone)
	{
		printf("[bt_gatt_client_clone] !clone\n");
		return NULL;
	}
