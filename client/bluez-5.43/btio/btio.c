...
// BEGIN ++++++++++++++++++++++++++++++++++++++++++++++++++++++++
static gboolean remote_connect_cb(GIOChannel *io,
								  GIOCondition cond,
								  gpointer user_data)
{
	printf("NATALYA : [btio.c::connect_cb]\n");
	struct connect *conn = user_data;
	conn->connect(NULL, NULL, conn->user_data);
							...
							gpointer user_data)
{
	...
	if (getsockopt(sock, SOL_SOCKET, SO_ERROR, &sk_err, &len) < 0){
		err = -errno;
//		printf("NATALYA DBG: connect_cb: getsockopt(...) <0, err = %d errno = %d\n", err, errno);
	}
	else{
		err = -sk_err;
//		printf("NATALYA DBG: connect_cb: getsockopt(...) >=0, err = %d\n", err);
	}

	if (err < 0){
		ERROR_FAILED(&gerr, "connect error", -err);
//		printf("NATALYA DBG: connect error = %d\n", -err);
	}
...
// BEGIN +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
static void remote_connect_add(BtIOConnect connect,
							   gpointer user_data,
							   GDestroyNotify destroy)
{
	printf("NATALYA : [remote_connect_add]\n");
	struct connect *conn;
	...
	remote_connect_cb(NULL, G_IO_OUT, conn);
}
// END +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
// BEGIN +++++++++++++++++++++++++++++++++++++++++++
void bt_io_remote_connect(BtIOConnect connect,
						  gpointer user_data,
						  GDestroyNotify destroy,
						  GError **gerr)
{
	remote_connect_add(connect, user_data, destroy);
}
// END +++++++++++++++++++++++++++++++++++++++++++
				...
				BtIOOption opt1, ...)
{
	...
	case BT_IO_L2CAP:
		err = l2cap_connect(sock, &opts.dst, opts.dst_type,
							opts.psm, opts.cid);
//		printf("NATALYA DBG: bt_io_connect : BT_IO_L2CAP err = %d\n", err);
		break;
	case BT_IO_RFCOMM:
		err = rfcomm_connect(sock, &opts.dst, opts.channel);
//		printf("NATALYA DBG: bt_io_connect : BT_IO_RFCOMM err = %d\n", err);
		break;
	case BT_IO_SCO:
		err = sco_connect(sock, &opts.dst);
//		printf("NATALYA DBG: bt_io_connect : BT_IO_SCO err = %d\n", err);
		break;
	case BT_IO_INVALID:
	default:
		g_set_error(gerr, BT_IO_ERROR, EINVAL,
					"Unknown BtIO type %d", opts.type);
//		printf("NATALYA DBG: bt_io_connect : Unknown BtIO type err = %d\n", err);
		return NULL;
	}
