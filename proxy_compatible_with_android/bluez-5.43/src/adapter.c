...
// BEGIN ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// callback when stop discovery is completed
static stop_discovery_cb_t stop_discovery_cb = NULL;

void adapter_stop_discovery_cb_register(stop_discovery_cb_t cb)
{
	if(stop_discovery_cb == NULL)
		stop_discovery_cb = cb;
	else
		DBG("NATALYA DBG: stop_discovery_cb is already set!");
}

static void stop_discovery_cb_call(stop_discovery_cb_t cb,
								   void *user_data)
{
	DBG("NATALYA DBG: call stop_discovery_cb_call callback");
	if(cb != NULL)
		(*cb)(user_data);
	else
		DBG("NATALYA DBG: stop_discovery_cb_call callback is not set!");
}
// END ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					...
					const void *param, void *user_data)
{
	...
	if (status == MGMT_STATUS_SUCCESS) {
		...
		stop_discovery_cb_call(stop_discovery_cb, adapter);
		trigger_passive_scanning(adapter);
	}
}
...
// BEGIN ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
DBusMessage *start_remote_discovery(DBusConnection *conn,
		DBusMessage *msg, void *user_data)
{
	struct btd_adapter *adapter = user_data;
	char addr_str[18];
	ba2str(&adapter->bdaddr, addr_str);
	DBG("NATALYA DBG: start_remote_discovery, adapter = %s, pointer = %p, addr = %s", adapter->dev_id, adapter, addr_str);

	return start_discovery(conn, msg, adapter);
}
// END ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
// BEGIN ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
DBusMessage *stop_remote_discovery(DBusConnection *conn,
		DBusMessage *msg, void *user_data)
{
	return stop_discovery(conn, msg, user_data);
}
// END ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
// BEGIN ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
static device_found_cb_t device_found_cb = NULL;

void adapter_device_found_cb_register(device_found_cb_t cb)
{
	if(device_found_cb == NULL)
		device_found_cb = cb;
	else
		DBG("Device Found callback is already set\n");
}

static void device_found_cb_call(device_found_cb_t cb,
								 uint16_t index,
	 	 	 	  	  	  	  	 uint16_t length,
								 const void *param,
								 void *user_data)
{
	DBG("NATALYA DBG: call device_found callback");
	if(cb != NULL)
		(*cb)(index, length, param, user_data);
	else
		DBG("NATALYA DBG: device_found callback is not set!");
}
...
//	DBG("NATALYA DBG print name %s", eir_data.name);
...
//		const uint32_t uuid32 = {.data = {0x00, 0x00, 0xFF, 0xE5}};
		...
		DBG("NATALYA CALLBACK hci%u type %u discovering %u method %d", adapter->dev_id, ev->type,
						ev->discovering, adapter->filtered_discovery);

//		dev = adapter_create_device(adapter, &addr, BDADDR_LE_PUBLIC);
//		adapter->connect_list = g_slist_append(adapter->connect_list, dev);

//		DBG("NATALYA CALLBACK %s added to %s's connect_list", device_get_path(dev),
//									adapter->system_name);
...
//			DBG("NATALYA DBG: data[0] = %d, data[1] = %d, data[2] = %c, data[3] = %c, data[4] = %c, data[5] = %c",
...
static void device_found_callback(uint16_t index, uint16_t length,
					const void *param, void *user_data)
{
//	DBG("NATALYA: Device found callback");
	...
	device_found_cb_call(device_found_cb, index, length, param, adapter);
}
					...
					uint8_t addr_type, uint8_t io_cap)
{
	...
	DBG("NATALYA DBG: adapter_bonding_attempt, sending MGMT_OP_PAIR_DEVICE");
	...
	if (id == 0) {
		...
		DBG("NATALYA DBG: adapter_bonding_attempt, FAILED TO PAIR");
		return -EIO;
	}
...
// BEGIN ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
static connect_ev_cb_t connect_cb = NULL;

void adapter_connect_ev_cb_register(connect_ev_cb_t cb)
{
	if(connect_cb == NULL)
		connect_cb = cb;
	else
		DBG("NATALYA DBG Connect_ev callback is already set!");
}

static void adapter_connect_ev_cb_call(connect_ev_cb_t cb,
									   uint16_t index,
									   uint16_t length,
									   const void *param,
									   void *user_data)
{
	if(cb != NULL)
		...
		DBG("NATALYA DBG: Connect_ev callback is not set!");
}
// END ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
static void connected_callback(uint16_t index, uint16_t length,
					const void *param, void *user_data)
{
	DBG("NATALYA DBG: CONNECTED CALLBACK !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
	...
	adapter_connect_ev_cb_call(connect_cb, index,length, param, adapter);
}
