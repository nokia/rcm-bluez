...
// HUI +++++++++++++++++++++++++++++++++++++++
void remote_eir_parse(struct eir_data *eir,
					  const uint8_t *eir_data,
					  uint8_t eir_len, char *addr,
					  bool in_use)
{
	uint16_t len = 0;
	char *label;
	...
	while (len < eir_len - 1) {
		...
		case EIR_NAME_COMPLETE:
			...
			if(in_use)
				...
				label = "(Remote)";
			char *underline = "_";
			char *dev_name = name2utf8(data, data_len);
			char *result = malloc(strlen(label)+strlen(addr)+strlen(dev_name)+2);
			strcpy(result, label);
			strcat(result, dev_name);
			strcat(result, underline);
			eir->name = strcat(result, addr);
    ...
    //HUI: if eir->name is NULL, create a new one
    if(!eir->name)
    {
    	printf("HUI: CREATE NEW EIR NAME!!!\n");
    	if(in_use)
    		label = "(IN_USE)(Remote)Unknown device_";
    	else
    		label = "(Remote)Unknown device_";
		char *result = malloc(strlen(label)+strlen(addr)+1);
		strcpy(result, label);
		eir->name = strcat(result, addr);
    }
}
