...
static DBusMessage *descriptor_write_value(DBusConnection *conn,
					DBusMessage *msg, void *user_data)
{
	printf("[descriptor_write_value] Entered\n");
	...
	printf("[descriptor_write_value] Start writing\n");
...
// BEGIN +++++++++++++++++++++++++++++++++++++++++++++++++
char_write_cb_t char_write_cb = NULL;

void char_write_cb_register(char_write_cb_t cb)
{
	DBG("NATALYA DBG: setting write char callback");
	if(char_write_cb == NULL)
		char_write_cb = cb;
	else
		DBG("NATALYA DBG: write char cb is already set!");
}

static void char_write_cb_call(char_write_cb_t cb, const char* sender, char* path, int value_len, uint8_t *value, struct btd_device *device)
{
	DBG("NATALYA DBG: char write callback call");
	if(cb != NULL)
		(*cb)(sender, path, value_len, value, device);
	else
		DBG("NATALYA DBG: char write callback is not set!");
}

static void display_reply_hex(ssize_t num_recv, uint8_t *server_reply)
{
	printf("Got %zu bytes\n", num_recv);
	ssize_t i;
					...
					DBusMessage *msg, void *user_data)
{
	 ...
	 */
	DBG("[characteristic_write_value] Start writing... BT_GATT_CHRC_EXT_PROP_RELIABLE_WRITE");
	display_reply_hex(value_len, value);
	// Managing the remote devices
	if(device_from_rcm(chrc->service->client->device) &&
	   !device_from_bluetooth(chrc->service->client->device))
	{
		//call the callback for remote devices
		// TODO: pass in this function the chrc->path as string, value and its length
		// modify the interface of the callback
		DBG("[characteristic_write_value] device_is_remote returned true\n");
		char_write_cb_call(char_write_cb, sender, chrc->path, value_len, value, chrc->service->client->device);
		return NULL; //dbus_message_new_method_return(msg);
	}
	else if(device_from_bluetooth(chrc->service->client->device))
	{
		DBG("[characteristic_write_value] device_is_remote returned false\n");
		...
		DBG("[characteristic_write_value] Start writing... BT_GATT_CHRC_PROP_WRITE");
		...
		if (chrc->props & BT_GATT_CHRC_PROP_WRITE) {
			...
			if (!mtu)
			{
				DBG("[characteristic_write_value] No ATT transport");
				return btd_error_failed(msg, "No ATT transport");
			}

			DBG("[characteristic_write_value] value_le = %d mtu = %d", value_len, mtu);
		...
		DBG("[characteristic_write_value] BT_GATT_CHRC_PROP_WRITE_WITHOUT_RESP");
...
fail:
	DBG("[characteristic_write_value] Failed to initiate write");
...
struct btd_gatt_client *btd_gatt_client_new(struct btd_device *device)
{
	...
	if (!device)
	{
		DBG("[src/gatt_client.c: btd_gatt_client_new] !device");
		return NULL;
	}
	...
	if (!db)
	{
		DBG("[src/gatt_client.c: btd_gatt_client_new] !db");
		return NULL;
	}
	...
	DBG("[src/gatt_client.c: btd_gatt_client_new] GATT client successfully created");
	return client;
}
...
//BEGIN
void btd_gatt_remote_client_ready(struct btd_gatt_client *client)
{
	if (!client)
		return;

	DBG("[btd_gatt_client_ready] client->device = %p", client->device);

	if (!client->gatt) {
		struct bt_gatt_client *gatt;

		gatt = btd_device_get_gatt_client(client->device);
		DBG("[btd_gatt_client_ready] gatt = %p", gatt);

		client->gatt = bt_gatt_remote_client_clone(gatt); // Difference is here, this function manages remote devices only!
		DBG("[btd_gatt_client_ready] client->gatt = %p", client->gatt);

		if (!client->gatt) {
			error("[btd_gatt_client_ready] GATT client not initialized");
			return;
		}
	}
...
void btd_gatt_client_connected(struct btd_gatt_client *client)
{
	...
	DBG("[btd_gatt_client_connected] client->device = %p", client->device);
	gatt = btd_device_get_gatt_client(client->device);
	if (!gatt) {
		error("[btd_gatt_client_connected] GATT client not initialized");
		return;
	}
