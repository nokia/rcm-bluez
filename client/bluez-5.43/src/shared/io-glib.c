...
ssize_t io_send(struct io *io, const struct iovec *iov, int iovcnt)
{
	...
	fd = io_get_fd(io);
//	printf("[io_send] NATALYA DBG: io->fd = %d iov[0]->base = %02X\n",
//					fd, /*io->events,*/ iov[0].iov_base);
	do {
		ret = writev(fd, iov, iovcnt);
//		printf("[io_send] NATALYA DBG inside while: ret = %d errno = %d io->fd = %d iov[0]->base = %02X\n",
//							ret, errno, fd, iov[0].iov_base);
