...
#include "btif/include/rcm_client.h"
...
void bta_gattc_open_error(tBTA_GATTC_CLCB* p_clcb,
                          UNUSED_ATTR tBTA_GATTC_DATA* p_data) {
  APPL_TRACE_ERROR("%s Connection already opened. wrong state", __func__);
...
void bta_gattc_open(tBTA_GATTC_CLCB* p_clcb, tBTA_GATTC_DATA* p_data) {
  ...
  APPL_TRACE_DEBUG("%s", __func__);
  // RCM
  if(rcm_device_exists(p_data->api_conn.remote_bda)){
	  rcm_start_remote_connect(p_data->api_conn.remote_bda, p_clcb);
	  ...
	  if (GATT_GetConnIdIfConnected(p_clcb->p_rcb->client_if, p_data->api_conn.remote_bda,
		  ...
		  // RCM
		  APPL_TRACE_DEBUG("RCM remotely connected : GATT_GetConnIdIfConnected");

		  bta_gattc_sm_execute(p_clcb, BTA_GATTC_INT_CONN_EVT, &gattc_data);
	  }
  }
  /* open/hold a connection */
  else if (!GATT_Connect(p_clcb->p_rcb->client_if, p_data->api_conn.remote_bda, true,
    ...
    if (GATT_GetConnIdIfConnected(
      ...
      // RCM
      APPL_TRACE_DEBUG("GATT_GetConnIdIfConnected");
...
void bta_gattc_init_bk_conn(tBTA_GATTC_API_OPEN* p_data,
  ...
  if (bta_gattc_mark_bg_conn(p_data->client_if, p_data->remote_bda, true)) {
    /* always call open to hold a connection */
	  if(rcm_device_exists(p_data->remote_bda)){
		  status = BTA_GATT_OK;
	  ...
	  else if (!GATT_Connect(p_data->client_if, p_data->remote_bda, false,
		  ...
		  APPL_TRACE_DEBUG("%s status BTA_GATT_OK", __func__);
...
void bta_gattc_write(tBTA_GATTC_CLCB* p_clcb, tBTA_GATTC_DATA* p_data) {
  if (!bta_gattc_enqueue(p_clcb, p_data)) return;
  APPL_TRACE_DEBUG("%s write_handle = %02x write_type = %d offset = %d",
		  __func__, p_data->api_write.handle, p_data->api_write.write_type, p_data->api_write.offset);
  ...
  if(rcm_device_exists(p_clcb->bda)){
	  status = rcm_remote_write(p_clcb, &attr, p_data);
  }else{
...
void bta_gattc_write_cmpl(tBTA_GATTC_CLCB* p_clcb, tBTA_GATTC_OP_CMPL* p_data) {
  ...
  APPL_TRACE_DEBUG("%s handle = %02x", __func__, p_data->p_cmpl->att_value.handle);
...
void bta_gattc_proc_other_indication(tBTA_GATTC_CLCB* p_clcb, uint8_t op,
                                     tGATT_CL_COMPLETE* p_data,
                                     tBTA_GATTC_NOTIFY* p_notify) {
  APPL_TRACE_DEBUG("%s: check p_data->att_value.handle=%d",
                   __func__, p_data->att_value.handle);
...
void bta_gattc_process_indicate(uint16_t conn_id, tGATTC_OPTYPE op,
  ...
  APPL_TRACE_DEBUG("%s handle = %02x", __func__, handle);
...
void bta_gattc_remote_process_indicate(uint16_t conn_id, tGATTC_OPTYPE op,
  ...
  tBTA_GATTC_NOTIFY notify;
//  RawAddress remote_bda;
  tBTA_GATTC_IF gatt_if = GATT_GET_GATT_IF(conn_id);
  tBTA_TRANSPORT transport = BT_TRANSPORT_LE;
...
static void bta_gattc_cmpl_cback(uint16_t conn_id, tGATTC_OPTYPE op,
                                 ...
                                 tGATT_CL_COMPLETE* p_data) {
  tBTA_GATTC_CLCB* p_clcb;
  APPL_TRACE_DEBUG("%s conn_id = %d op = %d status = %d handle = %02x",
                   __func__, conn_id, op, status, p_data->att_value.handle);

  for(int i=0; i < (p_data->att_value.len); i++)
	  APPL_TRACE_DEBUG("%s value_len = %d value[%d] = %02x", __func__, p_data->att_value.len, i, p_data->att_value.value[i]);
...
void bta_gattc_remote_cmpl_cback(uint16_t conn_id, tGATTC_OPTYPE op,
                                 ...
                                 tGATT_CL_COMPLETE* p_data, RawAddress &remote_bda) {
  tBTA_GATTC_CLCB* p_clcb;
  APPL_TRACE_DEBUG("%s conn_id = %d op = %d status = %d handle = %02x len = %d",
                   __func__, conn_id, op, status, p_data->att_value.handle, p_data->att_value.len);

  APPL_TRACE_DEBUG("%s p_data.att_value.conn_id=%d p_data.att_value.handle = %02x p_data.att_value.len=%d",
  ...
  /* notification and indication processed right away */
  if (op == GATTC_OPTYPE_NOTIFICATION || op == GATTC_OPTYPE_INDICATION) {
    bta_gattc_remote_process_indicate(conn_id, op, p_data, remote_bda);
    return;
  }
  /* for all other operation, not expected if w/o connection */
  else {
