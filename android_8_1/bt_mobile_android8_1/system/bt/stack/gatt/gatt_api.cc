...
bool GATT_Connect(tGATT_IF gatt_if, const RawAddress& bd_addr, bool is_direct,
  ...
  LOG(INFO) << __func__ << " is_direct=" << is_direct << " " << bd_addr;
  ...
  if (is_direct){
	  LOG(INFO) << __func__ << " gatt_act_connect" << " " << bd_addr;
    ...
    if (transport == BT_TRANSPORT_LE){
      status = gatt_update_auto_connect_dev(gatt_if, true, bd_addr);
  	  LOG(INFO) << __func__ << " gatt_update_auto_connect_dev, status = " << status << bd_addr;
