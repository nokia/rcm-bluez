...
static struct btd_device *adapter_create_remote_device(struct btd_adapter *adapter,
													   const bdaddr_t *bdaddr,
													   uint8_t bdaddr_type,
													   bool remote,
													   connect_cb_t cb)
{
	DBG("RCM DBG: [adapter_create_remote_device]");
	struct btd_device *device;

	device = device_create_remote(adapter, bdaddr, bdaddr_type, remote, cb);
	if (!device){
		return NULL;
	}
...
// BEGIN +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
static filter_cb_t set_filter_cb = NULL;
void adapter_set_discovery_filter_cb_register(filter_cb_t cb)
{
	DBG("RCM DBG: setting discovery filter callback");
	if(set_filter_cb == NULL)
		set_filter_cb = cb;
	else
		DBG("RCM DBG: setting discovery filter callback is already set!");
}

static void set_discovery_filter_cb_call(filter_cb_t cb, void *userdata, void *discovery_filter, const void *sender)
{
	DBG("RCM DBG: call set filter callback");
	if(cb != NULL)
		(*cb)(userdata, discovery_filter, sender);
	else
		DBG("RCM DBG: set filter callback is not set!");
}

static discovering_cb_t start_discovery_cb = NULL; // To distinguish passive and active discovery, better is to have a list of callback functions
static discovering_cb_t stop_discovery_cb = NULL;

void adapter_discovering_cb_register(discovering_cb_t cb)
{
	DBG("RCM DBG: setting discovering callback");
	if(start_discovery_cb == NULL)
		start_discovery_cb = cb;
	else
		DBG("RCM DBG: discovering callback is already set!");
}

static void discovering_cb_call(discovering_cb_t cb, void *adapter, const void *sender)
{
	DBG("RCM DBG: call discovering callback");
	if(cb != NULL)
		(*cb)(adapter, sender);
	else
		DBG("RCM DBG: discovering callback is not set!");
}

void adapter_stop_discovery_cb_register(discovering_cb_t cb)
{
	DBG("RCM DBG: setting stop discovery callback");
	if(stop_discovery_cb == NULL)
		stop_discovery_cb = cb;
	else
		DBG("RCM DBG: stop discovery callback is already set!");
}

// These two functions may be used to start/stop only the remote discovery (i.e. if the corresponding callbacks set)
// And in this case the real local discovery won't be started/stopped.
// To discuss... because not only remote devices should be detected
// but at the same time the errors due to local physical adapter
// should not impact the remote discovery...
static bool adapter_start_discovery_cb_set()
{
	if(start_discovery_cb != NULL)
		return true;
	return false;
...
static bool adapter_stop_discovery_cb_set()
{
	if(stop_discovery_cb != NULL)
		return true;
	return false;
...
//HUI
int cmp_device_id(gconstpointer a, gconstpointer b)
{
	return a == b ? 0 : -1;
}

void reset_device_flags(struct btd_adapter *adapter)
{
	GSList *list, *l;

	for(list = adapter->devices; list != NULL; list = list->next)
	{
		l = g_slist_find_custom(adapter->connections, list->data, cmp_device_id);

		//only reset flags for disconnected devices
		if(l == NULL)
		{
			device_set_rcm_flag(list->data, false);
			device_set_bluetooth_flag(list->data, false);
		}
	}
}
					...
					DBusMessage *msg, void *user_data)
{
	...
	//HUI
	printf("Before scanning, reset rcm & bluetooth flag\n");
	reset_device_flags(adapter);
	...
	if (client) {
		...
		// Trigger remote discovery if set
		discovering_cb_call(start_discovery_cb, adapter, sender);
					...
					DBusMessage *msg, void *user_data)
{
	...
	set_discovery_filter_cb_call(set_filter_cb, adapter, discovery_filter, sender);
					...
					DBusMessage *msg, void *user_data)
{
	...
	//++++++++++++++++++++++++++++
// Stop remote discovery only
//	if(adapter_stop_discovery_cb_set())
//	{
		discovering_cb_call(stop_discovery_cb, adapter, sender);
//		return dbus_message_new_method_return(msg);
//	}
	//++++++++++++++++++++++++++++
...
// BEGIN ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
void update_remotely_found_devices(struct btd_adapter *adapter,
					const bdaddr_t *bdaddr,
					...
					connect_cb_t cb)
{
	...
	remote_eir_parse(&eir_data, data, data_len, addr, in_use);
	//memcpy(&eir_data, data, sizeof(eir_data));
...
//	DBG("RCM DBG print device_addr = %s, device_name = %s", addr, eir_data.name);
	...
	if (!dev) {
		...
		dev = adapter_create_remote_device(adapter, bdaddr, bdaddr_type, true, cb);
	}
	...
	//HUI
//	DBG("HUI: Device from RCM!!!");
	device_set_rcm_flag(dev, true);
	device_set_callback(dev, cb);
...
//	DBG("RCM DBG print name %s", eir_data.name);
	...
	//HUI
//	DBG("RCM: Device from BLUETOOTH!!!");
	device_set_bluetooth_flag(dev, true);
...
/*
static void natalya_callback(uint16_t index, uint16_t length,
		const void *param, void *user_data)
{
	//struct btd_device *dev;
	const struct mgmt_ev_discovering *ev = param;
		...
		bdaddr_t addr;
		struct eir_data my_eir_data;
		uint8_t data[sizeof(my_eir_data)];
	//	char name[] = "Natalya-FAKE-LED";
		int len = 0;
		GSList *l;
		int i = 1;
//		uint8_t uuid[4] = {0x00, 0x00, 0xFF, 0xE5};
		uint8_t uuid[4] = {0xE5, 0xFF, 0x00, 0x00};

//		const uint32_t uuid32 = {.data = {0x00, 0x00, 0xFF, 0xE5}};

		memset(&my_eir_data, 0, sizeof(my_eir_data));

		str2ba(addr_str, &addr);
		DBG("RCM DBG: hci%u type %u discovering %u method %d", adapter->dev_id, ev->type,
						ev->discovering, adapter->filtered_discovery);

//		dev = adapter_create_device(adapter, &addr, BDADDR_LE_PUBLIC);
//		adapter->connect_list = g_slist_append(adapter->connect_list, dev);

//		DBG("RCM DBG %s added to %s's connect_list", device_get_path(dev),
//									adapter->system_name);
...
//											dev);
		...
		g_free(my_eir_data.name);
		my_eir_data.name = "Natalya-FAKE-LED";
		data[0] = strlen(my_eir_data.name) + 1;//sizeof(EIR_NAME_SHORT);
		data[1] = EIR_NAME_SHORT;
		memcpy(&data[2], my_eir_data.name, strlen(my_eir_data.name)*sizeof(char));
		// Move further
		len = strlen(my_eir_data.name) + 2;//sizeof(EIR_NAME_SHORT) + 1;
		// Initialize and write services
//		eir_data.services = g_slist_alloc();
		my_eir_data.services = g_slist_append(my_eir_data.services, "0000ffe5");//-0000-1000-8000-00805f9b34fb");

		printf("length %d, list length = %d\n", len, (int)g_slist_length(my_eir_data.services));
		data[len] = g_slist_length(my_eir_data.services)*4 + 1;//sizeof(EIR_UUID128_SOME);
		data[len + 1] = EIR_UUID32_ALL;//EIR_UUID128_SOME;

		for (l = my_eir_data.services; l != NULL; l = g_slist_next(l))
		{
...
//		memcpy(&data[2], name, sizeof(name));
//		memcpy(data + (sizeof(strlen(eir_data.name))+sizeof(EIR_NAME_SHORT))/sizeof(uint8_t), &eir_data.name, sizeof(strlen(eir_data.name)));

		for (uint8_t i=0; i<80; i++) {
		        printf("RCM DBG DATA [%02d] %02X %c\n", i, data[i], data[i]);
		}
//			DBG("RCM DBG: data[0] = %d, data[1] = %d, data[2] = %c, data[3] = %c, data[4] = %c, data[5] = %c",
//				data[0], data[1], (char)data[2], (char)data[3], data[4], (char)data[5]);
		update_found_devices(adapter,
							&addr,
							BDADDR_LE_PUBLIC, -55,
							false, true,
							false, data, sizeof(data));
							//(const uint8_t *) &eir_data, sizeof(eir_data));

//		device_add_eir_uuids(dev, uuids);
		DBG("RCM DBG hci%u type %u discovering %u method %d", adapter->dev_id, ev->type,
						ev->discovering, adapter->filtered_discovery);
}
*/
...
// MUST BE UNCOMMENTED!!!
// YOU CAN COMMENT IT TO BE SURE THAT THE DEVICE IS COMING FROM THE RCM
...
// BEGIN ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
static void remote_bonding_complete(struct btd_adapter *adapter,
					const bdaddr_t *bdaddr,
					uint8_t addr_type)
{
	struct btd_device *device;
	...
	if (device != NULL)
		device_remote_bonding_complete(device, addr_type);
	else
	{
		DBG("RCM DBG: [remote_bonding_complete] device doesn't exist!");
		return;
	}
	resume_discovery(adapter);

//	check_oob_bonding_complete(adapter, bdaddr, status); // XXX
}
// END ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
// BEGIN ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
void pair_remote_device_complete(const void *param,
								 void *user_data)
{
	...
	remote_bonding_complete(adapter, &rp->addr.bdaddr, rp->addr.type);
}
// END ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
// BEGIN +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
int adapter_remote_create_bonding(struct btd_adapter *adapter, const bdaddr_t *bdaddr,
					uint8_t addr_type, uint8_t io_cap)
{
	DBG("RCM DBG : [adapter_remote_create_bonding]");
/*// Normally we don't need this for remote pairing
 *
	...
	*/
	...
	return adapter_remote_bonding_attempt(adapter, bdaddr, addr_type, io_cap);
}
// END +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
// BEGIN ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
static pairing_cb_t pair_device_cb = NULL;

void adapter_pairing_cb_register(pairing_cb_t cb)
{
	if(pair_device_cb == NULL)
		pair_device_cb = cb;
	else
		DBG("RCM DBG: Pairing callback is already set!");
}

static void adapter_pairing_cb_call(pairing_cb_t cb, void *userdata)
{
	DBG("RCM DBG: call pairing callback");
	if(cb != NULL)
		(*cb)(userdata);
	else
		DBG("RCM DBG: pairing callback is not set!");
}

int adapter_remote_bonding_attempt(struct btd_adapter *adapter, const bdaddr_t *bdaddr,
					uint8_t addr_type, uint8_t io_cap)
{
	...
	DBG("RCM DBG: adapter_remote_bonding_attempt, sending MGMT_OP_PAIR_DEVICE");
// Should be managed by this callback
// Inside we should call the pair_device_complete function
// Somewhere else in the plugin (don't know yet at what moment) call the free_pair_device_data
	adapter_pairing_cb_call(pair_device_cb, data);
...
//	adapter->pair_device_id = 1; // Well, we don't have any id for the remote pairing.
								 // However, it may be useful to know that it is configured here in order it is checked somewhere else in the code.
					...
					uint8_t addr_type, uint8_t io_cap)
{
	...
	DBG("RCM DBG: adapter_bonding_attempt, sending MGMT_OP_PAIR_DEVICE");
	...
	if (id == 0) {
		...
		DBG("RCM DBG: adapter_bonding_attempt, FAILED TO PAIR");
		return -EIO;
	}
...
void adapter_disconnect_cb_register(disconnect_cb_t cb)
{
	DBG("RCM DBG: setting disconnect callback");
	if(disconnect_cb == NULL)
		disconnect_cb = cb;
	else
		DBG("RCM DBG: disconnect callback is already set!");
}

static void disconnect_cb_call(disconnect_cb_t cb, void *userdata)
{
	DBG("RCM DBG: call disconnect callback");
	if(cb != NULL)
		(*cb)(userdata);
	else
		DBG("RCM DBG: disconnect callback is not set!");
}
					...
					uint8_t reason)
{
	...
	disconnect_cb_call(disconnect_cb, device);
}
...
//BEGIN ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
void remotely_connected_device(char *addr,
							   uint8_t bdaddr_type,
							   const uint8_t *eir,
							   uint16_t eir_len,
							   void *user_data)
...
//	ba2str(&bdaddr, addr);
	DBG("RCM DBG: remotely connected device: addr = %s", addr);

	bdaddr_t bdaddr;
	str2ba(addr, &bdaddr);
	device = btd_adapter_find_device(adapter, &bdaddr,
								bdaddr_type);
	// !!! NB: if device is not found this function will create a new one for you!
	// And it won't be the remote device, so on this point be sure that your device is already created and exists!
...
//	send_dbus_message(device);
	...
	adapter_add_connection(adapter, device, bdaddr_type);
...
//HUI
static connect_cb_t local_connected_cb = NULL;

void adapter_local_connected_cb_register(connect_cb_t cb)
{
	DBG("RCM DBG: setting local connected callback");
	if(local_connected_cb == NULL)
		local_connected_cb = cb;
	else
		DBG("RCM DBG: local connected callback is already set!");
}

static void local_connected_cb_call(connect_cb_t cb, void *userdata)
{
	DBG("RCM DBG: call local connected callback");
	if(cb != NULL)
		(*cb)(userdata);
	else
		DBG("RCM DBG: local connected callback is not set!");
}
					...
					const void *param, void *user_data)
{
	...
	//HUI: inform proxy when device connects with bluetooth successfully
	local_connected_cb_call(local_connected_cb, device);
}
