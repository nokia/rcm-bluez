							...
							void *user_data)
{
	...
	DBG("NATALYA DBG: dev_connect, got DBus \"Connect\" message. device is remote? %d", dev->remote);

	if(!dev->remote)
	{
		uint8_t bdaddr_type;

		DBG("NATALYA DBG: dev_connect, device is not remote! = %d", dev->remote);
	...
	else // Device is a remote one, a connection should be managed differently
	{
		connect_cb_call(dev->connect_callback, dev);
		DBG("NATALYA DBG: dev_connect, device is remote %d, we shouldn't follow the standart connect process", dev->remote);
	}
	return NULL;
}

// BEGIN +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
DBusMessage *remote_dev_connect(DBusConnection *conn, DBusMessage *msg,
							void *user_data)
{
	return dev_connect(conn, msg, user_data);
}
...
// BEGIN +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
connect_cb_t connection_completed = NULL;

void device_connection_completed_cb_register(connect_cb_t cb)
{
	DBG("NATALYA DBG: setting connect callback");
	connection_completed = cb;
}

static void connection_completed_cb_call(connect_cb_t cb, void *userdata)
{
	DBG("NATALYA DBG: register connection_completed callback");
	if(cb != NULL)
		(*cb)(userdata);
	else
		DBG("NATALYA DBG: connection_completed callback is not set!");
}
// END +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
								...
								void *user_data)
{
	...
	// call the connection_completed callback here
	// Inside this function we should send the cache file containing the necessary info to load the correct database
	// located in /var/lib/bluetooth/adapter_addr/cache/device_addr
	connection_completed_cb_call(connection_completed, device);
}
