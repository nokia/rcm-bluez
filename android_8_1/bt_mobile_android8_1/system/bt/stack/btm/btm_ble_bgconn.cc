...
bool btm_ble_start_auto_conn(bool start) {
  ...
  // RCM: if a connection is already in progress through RCM, do not call btm_send_hci_create_connection
  if(rcm_connection_in_progress(RawAddress::kEmpty)){
	  BTM_TRACE_DEBUG("%s RCM connection in progress", __func__);
	  return exec;
  }
...
bool btm_ble_resume_bg_conn(void) {
  ...
  if (p_cb->bg_conn_type == BTM_BLE_CONN_AUTO) {
    bool ok = btm_ble_start_auto_conn(true);
    BTM_TRACE_DEBUG("%s returned %d", __func__, ok);
    return ok;
  }
