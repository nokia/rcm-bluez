...
// RCM callbacks
discovering_cb_t start_discovery_cb = NULL; // To distinguish passive and active discovery, better is to have a list of callback functions
discovering_cb_t stop_discovery_cb = NULL;

void rcm_adapter_discovering_cb_register(discovering_cb_t cb)
{
	APPL_TRACE_API("%s discovering cb register ", __func__);
	if(start_discovery_cb == NULL)
		start_discovery_cb = cb;
	else
		APPL_TRACE_API("%s discovery callback is not set!", __func__);
}

void rcm_adapter_stop_discovery_cb_register(discovering_cb_t cb)
{
	APPL_TRACE_API("%s stop discovery cb register ", __func__);
	if(stop_discovery_cb == NULL)
		stop_discovery_cb = cb;
	else
		APPL_TRACE_API("%s stop discovery callback is not set!", __func__);
}

static void rcm_discovering_cb_call(discovering_cb_t cb, bool start)
{
	APPL_TRACE_API("%s call discovery cb", __func__);
	if(cb != NULL)
		(*cb)(start);
	else
		APPL_TRACE_API("%s discovery callback is not set!", __func__);
...
extern void BTA_DmBleObserve(bool start, uint8_t duration,
  ...
  // RCM callback
  rcm_discovering_cb_call(start_discovery_cb, start);
