...
void BTM_BleStartAutoConn() {
  ...
  if (btm_cb.ble_ctr_cb.bg_conn_type != BTM_BLE_CONN_AUTO) {
    ...
    BTM_TRACE_DEBUG("%s btm_ble_start_auto_conn = %d", __func__, ok);
...
void btm_ble_process_adv_pkt(uint8_t data_len, uint8_t* data) {
	// RCM
	BTM_TRACE_DEBUG("btm_ble_process_adv_pkt");
...
static void btm_ble_process_adv_pkt_cont(
    ...
    uint8_t* data) {
	//RCM
	BTM_TRACE_DEBUG("btm_ble_process_adv_pkt_cont");
...
tBTM_STATUS btm_ble_start_scan(void) {
  BTM_TRACE_EVENT("btm_ble_start_scan ");
