...
#endif
	APPL_TRACE_DEBUG("%s s_handle=%d e_handle=%d uuidlen=%d primary=%d", __func__, s_handle, e_handle, p_uuid->len, is_primary);
	print_hex(p_uuid->len, &(p_uuid->uu));
...
//#if (BTA_GATT_DEBUG == TRUE)
  ...
  APPL_TRACE_DEBUG("attr_handle = %d handle=%d uuid16=0x%x property=0x%x", attr_handle, value_handle,
...
void bta_gattc_search_service(tBTA_GATTC_CLCB* p_clcb, tBT_UUID* p_uuid) {
	APPL_TRACE_DEBUG("%s ", __func__);
	if(p_uuid)
		APPL_TRACE_DEBUG("p_uuid len = %d", p_uuid->len);
  ...
  // RCM print
//  bta_gattc_display_cache_server(p_clcb->p_srcb->p_srvc_cache);
...
tBTA_GATTC_SERVICE* bta_gattc_find_matching_service(const list_t* services,
                                                    uint16_t handle) {
	APPL_TRACE_DEBUG("%s %02x", __func__, handle);
