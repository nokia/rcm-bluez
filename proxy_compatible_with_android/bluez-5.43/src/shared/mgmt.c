...
//	printf("[notify_handler] NATALYA DBG: mgmt_notify, event %d \n", notify->event);
//	printf("[notify_handler] NATALYA DBG: event_index, event %d length %d\n", match->event, match->length);

	if (notify->removed)
	{
//		printf("[notify_handler] NATALYA DBG: notify->removed\n");
		return;
	}

	if (notify->event != match->event)
	{
//		printf("[notify_handler] NATALYA DBG: notify->event != match->event\n");
		return;
	}

	if (notify->index != match->index && notify->index != MGMT_INDEX_NONE)
	{
//		printf("[notify_handler] NATALYA DBG: notify->index != match->index && notify->index != MGMT_INDEX_NONE\n");
		return;
	}

//	printf("[notify_handler] NATALYA DBG: Match!\n");
...
//	printf("[can_read_data] NATALYA DBG: can_read_data, event %d length %d\n", event, length);
