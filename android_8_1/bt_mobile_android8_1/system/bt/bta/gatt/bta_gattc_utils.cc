...
bool bta_gattc_uuid_compare(const tBT_UUID* p_src, const tBT_UUID* p_tar,
  ...
  if (p_src->len == LEN_UUID_16) {
	...
	APPL_TRACE_DEBUG("%s convert p_src 0x%04x to uuid128", __func__, p_src->uu.uuid16);
  ...
  if (p_tar->len == LEN_UUID_16) {
    /* convert a 16 bits UUID to 128 bits value */
	APPL_TRACE_DEBUG("%s convert p_tar(from cache) 0x%04x to uuid128", __func__, p_tar->uu.uuid16);
    bta_gatt_convert_uuid16_to_uuid128(tu, p_tar->uu.uuid16);
    pt = tu;
  } else
    pt = p_tar->uu.uuid128;

  APPL_TRACE_DEBUG("%s p_src", __func__);
  print_hex(LEN_UUID_128, su);
  APPL_TRACE_DEBUG("%s p_tar", __func__);
  print_hex(LEN_UUID_128, tu);

//  for(int i=0; i<LEN_UUID_128; i++)
//		APPL_TRACE_DEBUG("%s su[%d] = tu[%d] ? %02x = %02x", __func__, i, su[i], tu[i]);
